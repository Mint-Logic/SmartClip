            ...
            li.innerHTML = `
                ...
                <div class="item-footer">
                    <div style="display:flex; gap:10px; align-items:center;">
                        ...
                        <span class="clip-label click-only" style="font-size:0.55rem; color:var(--app-theme); cursor:pointer; font-weight:bold; opacity:${item.label ? '1' : '0.3'}; transition: opacity 0.2s;" title="${item.label ? 'Edit Label' : 'Set Custom Label'}"><i class="fa-solid fa-tag"></i> ${item.label || 'Add Label'}</span>
                    </div>
...
if (window.smartClip && window.smartClip.onLicenseResponse) {
    window.smartClip.onLicenseResponse((response) => {
        if (response.success) {
            ...
            Utils.showMsg("ERROR: " + response.reason);
            if (btnActivate) {
...
if (btnCheckUpd) {
    btnCheckUpd.onclick = () => {
        // Point directly to the Customer Portal for manual binary downloads
...
if (btnGetPro) {
    btnGetPro.onclick = () => {
        // Direct to Lemon Squeezy A-la-Carte Checkout for SmartClip
...
// 3. Listen for the validation response
window.smartClip.onLicenseResponse((response) => {
    if (response.success) {
        Utils.showMsg("PRO ACTIVATED! RESTARTING...");
        setTimeout(() => window.location.reload(), 1500);
...
// [FIX] Horizontal stretching disabled. UI Scale handles proportional sizing.
